...
void main() {
  ...
  group('AttendanceScreen', () {
    ...
    testWidgets('should display all required components', (WidgetTester tester) async {
      ...
      expect(find.text('勤怠管理'), findsOneWidget);
      expect(find.text('出勤スタッフの勤務時間を記録'), findsOneWidget);
      expect(find.byIcon(Icons.calendar_today), findsOneWidget); // DateNavigator
    ...
    testWidgets('should display correct department information', (WidgetTester tester) async {
      ...
      expect(find.textContaining('営業部'), findsAtLeastNWidgets(2)); // 田中、佐藤
      expect(find.textContaining('マーケティング部'), findsAtLeastNWidgets(2)); // 山田、鈴木
      expect(find.textContaining('技術部'), findsAtLeastNWidgets(2)); // 高橋、小林
      expect(find.textContaining('人事部'), findsAtLeastNWidgets(2)); // 渡辺、加藤
      expect(find.textContaining('経理部'), findsAtLeastNWidgets(2)); // 井上、斎藤
    });
  });
}
